		 ...
		 broadcast_system_t sys)
{
  ...
  //store encr_prod
  // FIX : the encryption product is not available at this time
  ...
  for(i = 0; i < 2*gbp->num_users; i++) {
    if(i == gbp->num_users) continue;
    out(gbp->gs[i], sysp);
    //printf("Size of g element in bytes : %d\n", element_length_in_bytes(gbp->gs[i]));
  ...
  for(i = 0; i < 2*gbp->num_users; i++) {
    if(i == gbp->num_users) continue;
    out(gbp->hs[i], sysp);
    //printf("Size of h element in bytes : %d\n", element_length_in_bytes(gbp->hs[i]));
		...
		broadcast_system_t *sys)
{
  ...
  // stefan fix : +++++++++++++++++
    char ss[1024];
    size_t count = fread(ss, 1, 1024, params);
    if (!count) pbc_die("input error");
    if (pairing_init_set_buf(p->pairing, ss, count)) pbc_die("pairing init failed");
  ...
  //restore encr_prod
  // Fix stefan: we don;t save the encrypted product at this time
  //element_init(s->encr_prod, p->pairing->G1);
  //in(s->encr_prod, sysp);
  //element_out_str(stdout, 0, s->encr_prod);
